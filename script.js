...
class BackgroundAudioManager {
    ...
    startSilentAudio() {
        try {
            ...
            // Create an almost silent oscillator
            const oscillator = this.audioContext.createOscillator();
            const gainNode = this.audioContext.createGain();

            // Very low frequency (almost DC)
            oscillator.frequency.setValueAtTime(0.001, this.audioContext.currentTime);
            
            // Extremely low gain (inaudible)
            gainNode.gain.setValueAtTime(0.000001, this.audioContext.currentTime);
            ...
            this.keepAliveInterval = setInterval(() => {
                if (this.audioContext && this.audioContext.state === 'suspended') {
                    ...
                    console.log('🔊 AudioContext resumed (background)');
                }
                
                // Also keep WebRTC audio track active
                this.keepWebRTCAudioActive();
            }, 15000);

            console.log('✅ Silent audio started for background');
        } catch (error) {
            console.log('❌ Silent audio error:', error);
        }
    }

    keepWebRTCAudioActive() {
        // Use the GLOBAL variables from main script
        if (window.localStream && window.isConnected) {
            const audioTracks = window.localStream.getAudioTracks();
            if (audioTracks.length > 0) {
                // Don't toggle if mic is locked
                if (window.isMicLocked) return;
                
                // Temporarily enable/disable to keep stream active
    ...
    handleVisibilityChange() {
        ...
        if (this.isBackground) {
            console.log('📱 App in background - maintaining connection');
            
            // Ensure audio stays active
            this.forceAudioResume();
            
            // Send background status to peer
            this.sendBackgroundStatus();
        } else {
...
// Make variables globally accessible for background manager
window.localStream = localStream;
window.isConnected = isConnected;
window.dataChannel = dataChannel;
window.myId = myId;
window.isMicLocked = isMicLocked;
...
function initializeWalkieTalkie() {
    ...
    // ✅✅✅ ADD THIS LINE - Initialize background audio
...
window.resetTalkButton = function() {
    if (talkButton) {
        ...
        // ✅✅✅ রিসেট করলেও মাইক বন্ধ রাখুন (Hold to Talk মোড)
        if (localStream) {
            localStream.getAudioTracks().forEach(track => {
                track.enabled = false;
            });
    ...
    async function answerCall(offerData) {
        try {
            ...
            // এই কোডগুলো খুঁজে বের করুন এবং ঠিক করুন:

// ইনকামিং কল answer করলে (লাইন ~827 এর কাছাকাছি)
        ...
        const playAudio = () => {
            remoteAudio.play()
                .then(() => {
                    ...
                    // ✅✅✅ এখানে মাইক বন্ধ রাখুন (Hold to Talk মোড)
                    if (localStream) {
                        localStream.getAudioTracks().forEach(track => {
                            track.enabled = false; // Hold to Talk মোডে মাইক বন্ধ
                        });
                    }
...
talkButton.addEventListener('mousedown', () => {
    ...
    if (localStream && isConnected) {
        ...
        // ✅✅✅ মাইক অন করুন শুধু যখন হোল্ড করবে
        localStream.getAudioTracks().forEach(track => {
            track.enabled = true; // হোল্ড করলে মাইক অন
        });
...
talkButton.addEventListener('mouseup', () => {
    ...
    if (localStream && isConnected) {
        ...
        // ✅✅✅ মাইক বন্ধ করুন যখন হোল্ড ছাড়বে
        localStream.getAudioTracks().forEach(track => {
            track.enabled = false; // হোল্ড ছাড়লে মাইক বন্ধ
        });
...
function toggleMicLock() {
    ...
    if (!isMicLocked) {
        // LOCK MIC (Turn ON)
        isMicLocked = true;
        track.enabled = true; // লক করলে মাইক অন
        isTalking = true;
        isListening = false;
        ...
        updateStatus("success", "fas fa-microphone-alt", "Mic Locked ON", 
                   "Microphone is locked in talking mode. Double tap to unlock.");
        
        console.log("🔒 MIC LOCKED ON");
        ...
        // UNLOCK MIC (Turn OFF)
        isMicLocked = false;
        track.enabled = false; // আনলক করলে মাইক বন্ধ (Hold to Talk মোড)
        isTalking = false;
        isListening = true;
    ...
    window.fullCleanup = function() {
        ...
        // ✅✅✅ ADD THIS - Cleanup background audio
